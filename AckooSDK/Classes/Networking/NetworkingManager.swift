    ...
    func validateResponse(_ data: Data?, response: URLResponse?, error: Error?, callback: (_ succeeded: Bool, _ response: Any?) -> Void) {
        ...
        do {
            ...
            let json = try JSONSerialization.jsonObject(with: data, options: [])
                ////print("\(response)")
            if let jsonDict: [String: AnyObject] = json as? [String: AnyObject] {
                if jsonDict.count == 0 {
                    callback(false, [:])
                    return
                }
            ...
            if let resDict: [String: Any] = json as? [String: Any] /* , let sON:Dictionary<String,Any> = resDict as?  Dictionary<String,Any> */ {
                successCall(response: response, json: resDict, callback: callback)
            } else if let resArray: [[String: Any]] = json as? [[String: Any]] /* , let sON:Array<[String:Any]> = resArray as?  Array<[String:Any]> */ {
